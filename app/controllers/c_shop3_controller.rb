...
class CShop3Controller < ApplicationController
def test1
@a = "hello test"
end
def test
#@category=	Masterusers.title('sachin singh mathan')
#render "test1"
@aa = params[:controller]
...
#session[:master_user_id]=Masterusers.shop_search(request.subdomain);
		...
		if cookies[:recently_view]== nil then 
		 ...
		 if cookies[:recently_view].index(params[:product]) == nil then 
			...
			@related=Masterusers.find_by_sql("select * from product_#{session[:master_user_id]} where sub_category_id=#{@product_detail[0]['sub_category_id']} ORDER BY product_id DESC LIMIT 4"  )
