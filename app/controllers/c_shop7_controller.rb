class CShop7Controller < ApplicationController
...
@product=Masterusers.find_by_sql("select * from product_"+session[:master_user_id].to_s )
	...
	if params[:filter] == "subcategory"
		...
		@product=Masterusers.find_by_sql("select * from product_"+session[:master_user_id].to_s+" where sub_category_id = "+params[:s_id].to_s)

	end 
	if params[:filter] == "search"

		@product=Masterusers.find_by_sql("select * from product_"+session[:master_user_id].to_s+" where product_category_id = "+params[:c_id])
