...
if ( jigoshop_params.is_checkout ) {
	...
	jQuery(function(){
		...
		jQuery('form.checkout').submit(function(){
			...
			jQuery(form).find('span.required').parents('.form-row').find('input').addClass('required');
			...
			jQuery.ajax({
				...
				success: 	function( code ) {
					...
					try {
						...
						jQuery(form).unblock();
						jQuery('.jigoshop_error').text('All the fields with asterisk (*) are required.')
						jQuery.scrollTo(jQuery(form).parents('article'), 100, {easing:'swing'});
					}
				},
				dataType: 	"html"
			});
